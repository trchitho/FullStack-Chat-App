...
const ChatContextMenu = ({ position, items, onClose }) => {
  ...
  return createPortal(
    <div ref={menuRef} role="menu" className="fixed z-[250] min-w-64 rounded-xl border border-base-300 bg-base-100 p-1 shadow-2xl" style={{ left: position.x, top: position.y }}>
      {items.map((item) => (
        <button key={item.label} type="button" role="menuitem" disabled={item.disabled} className="flex min-h-11 w-full items-center justify-between rounded-lg px-3 text-left text-sm hover:bg-base-200 disabled:opacity-40" onClick={() => { item.action(); onClose(); }}>
